...
import { 
  Loader2, Calendar, User, Phone, Mail, Clock, MapPin, 
  Facebook, Instagram, Twitter, ArrowRight, ShieldCheck, 
  Ticket, UserPlus, Info, Briefcase, Map, 
  AlertTriangle
} from 'lucide-react';
    ...
    <div className="min-h-screen font-sans bg-slate-100 lg:bg-white lg:grid lg:grid-cols-2">
      ...
      <div className="hidden lg:flex relative flex-col justify-between bg-[#1B2541] text-white p-12 overflow-hidden h-screen sticky top-0">
         ...
         <div className="relative z-10 flex flex-col h-full justify-center max-w-xl">
             ...
             {/* FECHA Y UBICACIÓN (DESKTOP) */}
             <div className="mb-6 flex flex-wrap gap-3">
                 <div className="inline-flex items-center gap-2 bg-[#FFC400] text-[#1B2541] px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wider shadow-lg">
                 ...
                 {/* UBICACIÓN (NUEVO) */}
                 {event?.location && (
                     <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wider shadow-lg">
             ...
             {event?.description && (
                <div className="max-h-[30vh] overflow-y-auto pr-4 custom-scrollbar mb-8">
                    <p className="text-lg text-slate-200 leading-relaxed border-l-4 border-[#FFC400] pl-6">
                        {event.description}
                    </p>
                </div>
             )}
      ...
      <div className="flex flex-col items-center justify-center p-4 lg:p-12 bg-slate-100 lg:bg-white h-full min-h-screen overflow-y-auto">
         ...
         <Card className="w-full max-w-md shadow-xl lg:shadow-none lg:border-2 lg:border-slate-100 border-0 overflow-hidden rounded-2xl bg-white relative z-10">
            ...
            <CardContent className="p-6 lg:p-8">
               ...
               {/* DESCRIPCIÓN Y UBICACIÓN PARA MÓVIL */}
               {(event?.description || event?.location) && (
                   <div className="lg:hidden mb-6 bg-slate-50 p-4 rounded-xl border border-slate-100">
                       <h3 className="text-xs font-bold text-[#1B2541] uppercase flex items-center gap-2 mb-2">
                           <Info className="h-4 w-4 text-[#FFC400]" /> Información
                       </h3>
                       
                       {/* UBICACIÓN MÓVIL */}
                       {event?.location && (
                           <div className="flex items-center gap-2 text-sm font-bold text-[#1B2541] mb-2 pb-2 border-b border-slate-200">
                 ...
                 <form onSubmit={handleSubmit(confirmRegistration)} className="space-y-5 animate-in fade-in slide-in-from-right-4">
                    ...
                    {/* <div className="relative">
                        <Select onValueChange={(val) => setValue('occupation', val)}>
                            ...
                            <SelectContent>
                                {occupations.map((oc) => (
                                    <SelectItem key={oc.id} value={String(oc.id)}>{oc.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    ...
                    <div className="w-full bg-white border-x border-b border-slate-200 rounded-b-3xl p-6 relative shadow-2xl">
                        ...
                        <div className="flex justify-center my-4">
                            <div className="p-3 bg-white border-2 border-slate-100 rounded-xl shadow-inner">
                                <QRCode value={getQrValue()} size={160} fgColor="#1B2541" />
                            </div>
                        </div>
